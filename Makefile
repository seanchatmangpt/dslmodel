...
test-essential:
	...
	@poetry run pytest tests/test_cli_essential.py -v --tb=short
...
verify:
	...
	@echo "=============================================="
	@poetry run python -c "from dslmodel.cli import app; print('✅ dsl CLI import successful')" 2>/dev/null || echo "⚠️ dsl CLI import failed (OpenAI dependency)"
	@poetry run python -c "import typer; print('✅ CLI framework available')" 2>/dev/null || echo "⚠️ CLI framework missing"
...
test-quick:
	...
	@echo "========================"
	@timeout 120 poetry run pytest tests/test_cli_essential.py::TestCLIEssential::test_core_commands_smoke -v
	@timeout 120 poetry run pytest tests/test_cli_essential.py::TestCLIEssential::test_work_lifecycle_essential -v
...
test-unit:
	@echo "🔬 Unit Tests"
	@echo "============="
	@poetry run pytest tests/ -k "not integration and not performance" -v

# Integration tests only
test-integration:
	@echo "🔗 Integration Tests"
	@echo "==================="
	@poetry run pytest tests/test_cli_essential.py::TestCLIEssential::test_work_lifecycle_essential -v
	@poetry run pytest tests/test_cli_essential.py::TestCLIEssential::test_ai_integration_quick -v

# Performance baseline tests
test-performance:
	@echo "📈 Performance Baseline Tests"
	@echo "============================="
	@poetry run pytest tests/test_cli_essential.py -k "performance" -v
...
otel-validate:
	...
	@poetry run python -c "from dslmodel.agents.otel.otel_instrumentation import setup_telemetry; setup_telemetry('test'); print('✅ OTEL setup successful')" || echo "⚠️ OTEL setup failed"
	@poetry run pytest tests/test_cli_essential.py::TestCLIEssential::test_otel_instrumentation_quick -v
...
otel-demo:
	...
	@sleep 5
	@poetry run python src/dslmodel/examples/run_thesis_otel_loop.py
...
test-deps:
	...
	@echo "=============================="
	@poetry run python -c "import pytest; print('✅ pytest available')" || (echo "❌ pytest missing" && exit 1)
	@poetry run python -c "import typer; print('✅ typer available')" || (echo "❌ typer missing" && exit 1)
...
check-deps:
	...
	@echo ""
	@echo "Key packages:"
	@poetry run python -c "import sys, pkg_resources; [print(f'  ✅ {d.project_name} {d.version}') for d in pkg_resources.working_set if d.project_name in ['pytest', 'typer', 'pydantic', 'opentelemetry-api']]"
	@echo ""
	@echo "System tools:"
