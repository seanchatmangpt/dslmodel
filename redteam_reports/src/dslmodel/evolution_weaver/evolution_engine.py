...
try:
    ...
    if "prometheus" in str(e).lower():
        logger.info("📋 OTEL available but Prometheus exporter not installed - using OTLP only")
        try:
            from ..otel.otel_instrumentation import init_otel, get_otel
            OTEL_AVAILABLE = True
        except ImportError:
            logger.warning("⚠️  OTEL instrumentation unavailable - falling back to mock")
            OTEL_AVAILABLE = False
    else:
        logger.warning(f"⚠️  OTEL instrumentation unavailable: {e}")
        OTEL_AVAILABLE = False

# Fallback to mock if OTEL is not available
