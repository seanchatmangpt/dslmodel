...
fabricates fitness, reports synthetic deployment health, or merges without an
...
def _strategy(value: str) -> EvolutionStrategy:
    ...
    aliases = {
        ...
        "quality": EvolutionStrategy.RELIABILITY_IMPROVEMENT,
    ...
    try:
        ...
        raise typer.BadParameter(
            "strategy must be one of performance, quality, coordination, features, reliability, "
...
def experiment(
    ...
    actuate_merge: bool = typer.Option(
        False,
        "--actuate-merge",
        help="Actually merge after successful verification. Disabled by default.",
    ),
    ...
    try:
        ...
        if actuate_merge:
            ...
            receipt = engine.merge_successful_candidate(candidate.candidate_id, target_branch=merge_target)
            merge_receipt = asdict(receipt) | {"receipt_id": receipt.receipt_id}
            merged = True

        cleanup: dict[str, Any] | None = None
        if not keep_worktree:
            # An unmerged candidate branch is preserved for diagnosis/replay; only
            # a successfully merged topic branch may be deleted automatically.
            cleanup = engine.cleanup_candidate(candidate.candidate_id, delete_branch=merged)
...
def coordinate(
    ...
    """Validate a reversible portfolio of patch candidates without automatic actuation."""
    ...
    try:
        ...
        if not keep_worktrees:
            ...
            for candidate in list(generation.candidates):
                try:
                    cleanup_receipts.append(engine.cleanup_candidate(candidate.candidate_id, delete_branch=False))
                except EvolutionError as exc:
                    cleanup_errors.append({"candidate_id": candidate.candidate_id, "error": str(exc)})
            payload["cleanup_receipts"] = cleanup_receipts
